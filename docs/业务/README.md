...
> ⚠️ **`biz_demo_tag` 不在这张表里** —— 它是**多对多的样板表**（见 `docs/生成器/表与关联.md` §4），
> 不是业务模块：没有菜单，也从没生成过代码。建表脚本 `backend/sql/biz_demo_tag.sql` **要留着**
> （多对多的示例靠它）。
>
> 它以前出现在上面那张表里，是因为 `doc-scan` 把「**为了配关联而导入生成器的表**」
> 也当成了模块 —— 这类表有 `gen_table` 记录，但没有菜单，按它自己的规则本该排除。
>
> **而真正值得追的问题在别处**：演示产品的标签字段配着
> `选项来自接口 /biz/demoTag/list`，**但那个接口不存在**。
> 也就是说这个模块是**设计过、没实现** —— 详见 `AI工程化需求.txt` §3.28。
