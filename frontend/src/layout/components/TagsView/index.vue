...
.tags-view-container {
  ...
  // 亮色「卡片」样式的激活页签。
  // 注意：这条规则实际上是**兜底**，不是主路径 —— 激活页签的颜色由 JS 的
  // tagActiveStyle() 以内联样式下发（值来自 settingsStore.theme），内联样式优先级最高，
  // 所以这条规则在 card / chrome 两种样式下都不会真正命中。
  // 原值 #42b983 是硬编码的绿色，若哪天 JS 绑定被移除就会露出这个不跟随主题的颜色，
  // 因此这里改为 var(--el-color-primary) 作为与主题一致的兜底。
