...
export function tansParams(params: Record<string, any>): string {
  ...
  for (const propName of Object.keys(params)) {
    ...
    if (value !== null && value !== "" && typeof (value) !== "undefined") {
      if (Array.isArray(value)) {
        // 数组发成**重复参数**（tags=1&tags=2），不再发 tags[0]=1&tags[1]=2。
        // 后端的查询字段是 String，Spring 的 ArrayToStringConverter 会把重复参数合成 "1,2"
        // —— 逗号正是库里存的形状，Mapper 再按逗号拆开逐个 find_in_set。
        // 空数组什么也不发，等价于「这个条件没筛」，而不是「筛空值」。
