    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="字段信息" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          ...
          <!--
            「常用」= 生成的页面里该条件**默认显示**；不勾则收进「展开查看更多」。
            目标是默认显示的条件恰好占满一行（约 3 个）。

            ⚠️ 必须用 true-value/false-value 的字符串 "1"/"0"，与同排其它复选框一致 ——
            后端 GenTableColumn.isCommon(String) 比的正是 '1'；
            用裸布尔 v-model 会写进 true，所有 is_common = '1' 的判断全部失效。

            非查询列上勾「常用」没有意义（GenUtils 只在 isQuery 分支里设 isCommon，
            synchDb 也只对 isQuery 列保留），所以禁用而非隐藏，把约束显式说出来。
            但**不**在取消「查询」时自动清空 isCommon —— synchDb 会刻意保留它，
            这样"查询 → 取消 → 再勾上"往返不会丢掉原有配置。
          -->
          <el-table-column label="常用" min-width="5%">
            <template #default="scope">
              <el-checkbox true-value="1" false-value="0" v-model="scope.row.isCommon"
                           :disabled="scope.row.isQuery !== '1'"></el-checkbox>
            </template>
          </el-table-column>
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(res => {
    ...
    if (validateResult) {
      ...
      genTable.params = {
        ...
        softDelete: info.value.softDelete ? '1' : '0',
